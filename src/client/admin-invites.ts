...
async function loadAppsForInvite() {
	try {
		...
		appRolesContainer.innerHTML = data.clients
			...
			.map((app: { id: number; clientId: string; name: string; roles: string[] }) => {
...
async function submitCreateInvite() {
	...
	const expiresAtInput = document.getElementById('expiresAt') as HTMLInputElement;
	...
	const expiresAt = expiresAtInput.value ? Math.floor(new Date(expiresAtInput.value).getTime() / 1000) : null;
...
function closeCreateInviteModal() {
	...
	if (modal) {
		...
		const appRolesContainer = document.getElementById('appRolesContainer');
		if (appRolesContainer) {
			appRolesContainer.querySelectorAll('input[type="checkbox"]').forEach((input) => {
...
async function loadInvites() {
	try {
		...
		invitesList.innerHTML = data.invites.map((invite: {
			...
			createdBy: string;
			inviteUrl: string;
			appRoles: Array<{ clientId: string; name: string | null; role: string }>;
			...
			const roleInfo = invite.appRoles.length > 0
				? `<div class="invite-roles">App roles: ${invite.appRoles.map(r => {
...
(window as any).editInvite = async (inviteId: number) => {
	try {
		...
		const data = await response.json();
		const invite = data.invites.find((inv: { id: number }) => inv.id === inviteId);
		...
		// Populate form
		(document.getElementById('editMaxUses') as HTMLInputElement).value = String(invite.maxUses);
		(document.getElementById('editInviteNote') as HTMLTextAreaElement).value = invite.note || '';
		(document.getElementById('editInviteMessage') as HTMLTextAreaElement).value = invite.message || '';
		
		// Handle expiration date
		const expiresAtInput = document.getElementById('editExpiresAt') as HTMLInputElement;
		if (invite.expiresAt) {
...
(window as any).submitEditInvite = async () => {
	...
	const maxUsesInput = document.getElementById('editMaxUses') as HTMLInputElement;
	const expiresAtInput = document.getElementById('editExpiresAt') as HTMLInputElement;
	const noteInput = document.getElementById('editInviteNote') as HTMLTextAreaElement;
	const messageInput = document.getElementById('editInviteMessage') as HTMLTextAreaElement;
	const submitBtn = document.getElementById('submitEditInviteBtn') as HTMLButtonElement;
...
(window as any).closeEditInviteModal = () => {
	...
	if (modal) {
		...
		(document.getElementById('editInviteMessage') as HTMLTextAreaElement).value = '';
	}
};
